...
int define_metodo(int argc, char** argv) {
  int metodo = SELECAO;
  if (argc > 2) { // se tiver mais de 2 argumentos, encerra programa com erro
    std::cout << "*** Quantidade de parâmetros inválidos *** \n\n";
    print_usage(); // Imprime o manual de uso
    exit(1); // programa encerra com erros
  }
  if (argc == 2) { // se tiver dois argumentos
    if (argv[1][0] != '-') { // se primeiro char for '-'
      print_usage();
      exit(0); // programa encerra sem erros
    }
    else if (argv[1][1] == 's') {
      metodo = SELECAO;
